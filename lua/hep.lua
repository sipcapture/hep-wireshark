...
function dissect_hep1(buffer, offset, subtree, pinfo, tree)
    ...
    subtree:add(fds1.ip_family, buffer(offset, ONEOCTET), ip_family)
...
function hep1_proto_dissector(buffer, pinfo, tree)
    local subtree = tree:add(hep1_proto, buffer(), "HEP1 Protocol")
    dissect_hep1(buffer, offset, subtree, pinfo, tree)
end
...
function hep_proto.dissector(buffer, pinfo, tree)
    ...
    if (tostring(version) == "02") then
        ...
        return
    elseif (tostring(version) == "01") then
...
-- Register with HTTP media type for HEP over HTTP
...
-- Register with HTTP2 streaming dissector
http2_table = DissectorTable.get("http2.streamid")
if http2_table then
    -- HTTP2 streams will be identified by content-type header
    -- The http2 dissector will call media_type table based on content-type
end
